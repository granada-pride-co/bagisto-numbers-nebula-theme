                ...
                @foreach ($steps as $index => $step)
                    <div class="border border-[var(--section-color,#2e2224)] bg-white/70 p-6 flex flex-col justify-between relative group hover:border-[#bd1765] transition-colors">
                        <div>
                            <div class="flex items-center justify-between mb-4">
                                ...
                                @if (! empty($step['timing']))
                                    <span class="font-mono text-[10px] uppercase tracking-wider px-2 py-0.5 border border-[var(--section-color,#2e2224)]/40 opacity-80">
                                        {{ $step['timing'] }}
                                    </span>
                                @endif
                            </div>
                            ...
                            <h3 class="font-mono text-lg font-bold text-[var(--section-color,#2e2224)] mb-2">
                                {{ $step['product_name'] ?? '' }}
                            </h3>

                            @if (! empty($step['key_actives']))
                                <div class="bg-[var(--section-color,#2e2224)]/5 p-2 mb-3 border-s-2 border-[var(--section-color,#2e2224)]">
                                    <span class="font-mono text-[10px] text-[var(--section-color,#2e2224)] font-semibold block">
                                        {{ $step['key_actives'] }}
                                    </span>
                                </div>
                            @endif
                        ...
                        @if (! empty($step['product_link']))
                            <div class="mt-6 pt-4 border-t border-[var(--section-color,#2e2224)]/20">
                                <a href="{{ $step['product_link'] }}" class="font-mono text-xs font-bold tracking-wider text-[var(--section-color,#2e2224)] hover:text-[#bd1765] inline-flex items-center gap-1 group-hover:underline">
