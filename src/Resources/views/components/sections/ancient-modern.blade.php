...
<section class="border-b border-[var(--section-color,#2e2224)] py-16 px-6 md:px-12" id="ancient-modern" dir="{{ $isAr ? 'rtl' : 'ltr' }}" style="--section-bg: {{ $bgColor }}; --section-color: {{ $textColor }}; background-color: var(--section-bg); color: var(--section-color); border-color: var(--section-color);">
    <div class="max-w-7xl mx-auto">
        <div class="text-center max-w-3xl mx-auto mb-14 reveal">
            ...
            <p class="font-mono text-xs md:text-sm leading-relaxed opacity-85">
                {{ $description }}
            </p>
        </div>
        ...
        @if (! empty($cards) && is_array($cards))
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                @foreach ($cards as $card)
                    <div class="border border-[var(--section-color,#2e2224)] bg-white/60 p-6 flex flex-col justify-between transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                        <div>
                            @if (! empty($card['badge']))
                                <span class="inline-block font-mono text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 border border-[var(--section-color,#2e2224)] bg-[var(--section-color,#2e2224)] text-[var(--section-bg,#ffffff)] mb-4">
                                    {{ $card['badge'] }}
                                </span>
                            @endif
